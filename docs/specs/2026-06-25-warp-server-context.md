...
1. **Bootstrap — sourcing `TContext`'s model when building the server context.** `TContext`'s model is built lazily and isn't available at `AddWarpServer` (DI-registration) time. The server context's model build (or its `UseModel`) must resolve `TContext`'s `IModel` at first use. Options: a model customizer on the server context that resolves `TContext`'s `IModel` via the root provider / `IModelSource`, or a captured name-map built on first server-context resolution. No circular dependency (`TContext`'s model is independent), but the timing/wiring is the central risk.
