...
Shipped via `IWarpServerContext` (abstraction; components extract `DbContext Context` once) over an internal `WarpServerContext<TContext>`. **All** server-internal DB work — the job worker (fetch/execute/complete + monitor), the server tasks, and the background-service host — runs on it; only the outbox (publisher) and the handler scope stay on `TContext`. Quiet logging via `ConfigureWarnings` demoting `CommandExecuted` to `Debug` (opt-out: `EnableServerCommandLogging`). Validated: NoDb 572 / PostgreSQL 760 / SQL Server 754.

**Investigation note.** Moving the worker initially appeared to cause a ~5s graceful-cancellation regression. Instrumentation disproved that: cancellation detect→finalize→`Cancelled`-row commit was ~11ms; the failures were a **test-harness gap**. `WarpTestServer.WaitForJobLog` waited on a `JobLogObserver` EF interceptor attached only to `TestContext`, so worker `JobLog` writes via the server context were never observed and the deterministic wait timed out at 5s — though the row was committed (it appeared in the failure's own "Events:" dump). Fix: `WaitForJobLog` now polls the committed row (consistent with `WaitForJobState`), robust to whichever context wrote it; the `TContext`-only observer was removed. With that, the worker runs on the server context and both DB suites are green — so worker-fetch SQL is silenced too, not just the background polling.
