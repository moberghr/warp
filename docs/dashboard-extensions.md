...
1. **Backend (.NET)** — An `IWarpDashboardExtension` implementation declares a manifest and optionally registers API endpoints. The extension's JS file is shipped as an embedded resource and served by the middleware.
...
```
1. User registers IWarpDashboardExtension in DI
2. MapWarpDashboard() discovers extensions, serves their embedded JS at /_ext/{name}/
...
### 1. Implement IWarpDashboardExtension
