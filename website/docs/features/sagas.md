...
- **Policy addons don't apply to saga messages.** Since 6.0 the policy attributes (`[Mutex]` / `[Semaphore]`, `[RateLimit]`, `[Timeout]`, `[Retry]`, `[CircuitBreaker]`) *do* apply to ordinary message handlers — but not to saga ones. The saga proxy is an `IPolicyExemptHandler`: it serializes on its own per-correlation mutex and owns its busy / version-conflict reschedules, so every policy behavior skips it, and global defaults don't reach it either. Attaching one of these attributes to a saga message or a saga handler still has no effect — no error, no warning. If you need rate-limiting, timeouts, or extra mutual exclusion around saga work, apply the attribute to the child `IJob` types the saga publishes from inside the handler, not to the saga message or handler itself.
