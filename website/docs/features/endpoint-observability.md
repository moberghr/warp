...
- **Timing + outcome** — duration, the **final wire status code**, and outcome (`Failed` when the status is ≥ 500 or an exception escaped after the response had started, else `Success`). When a handler exception escapes before the response starts, recording is deferred until the response completes — so a host exception middleware that maps domain exceptions to statuses (e.g. `NotSignedInException` → 401) produces a row with the real 401, not a stale 200 default. Such a call is classified like any 4xx (a client error, kept off the error rate), with the exception type and message still stored on the row; it is always written even under `RecordCalls = FailuresOnly`, and `OnFailure` capture tiers treat it as capture-worthy.
