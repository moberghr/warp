...
3. **There is no longer any startup validation of policy placement, and declaring a policy on both axes is no longer an error.** `AddWarp` previously threw for the both-axes case, for `Scope = Total` on a handler, and for a handler `[Timeout]` under a `Total`-scoped global default. Both-axes is now a supported override. The remaining unresolvable placements fail the **build** where the compiler can see them, and fail the **job** (not the process) where it cannot: `[Timeout(Scope = Total)]` on a handler throws at execution, because a wall-clock budget measured from enqueue cannot be resolved once the job is already running.
