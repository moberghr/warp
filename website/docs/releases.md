...
Performance release, **no schema change and no migration**: a server now puts substantially less load
on the database it shares with your application. Measured against the previous release on an
otherwise identical workload, **database time per job fell 52-63% and statements per job 52-61%**,
...
**No migration.** No entity, column or index changes — the schema is byte-for-byte what 6.2 shipped.
Upgrading is a package bump.

**No behavioural break.** Job execution, state transitions, retries, scheduling and every
control-plane record are untouched. The one semantic change is the counter durability window
described below, and it applies to metrics only.

**One source-level break, and only if you construct Warp's worker types by hand.** Three public
constructors gained a required `WarpCounterBuffer` parameter:

- `WarpWorkerService<TContext>`
- `WarpSingleWorkerHost<TContext>`
- `WarpDispatcherHost<TContext>`

All three are registered and resolved by `AddWarpServer`, so a normal host — anything that configures
Warp through the builder — is unaffected and needs no change. You only hit this if you `new` one
...
the *same twenty keys* every time, so 100,000 jobs wrote two million rows that `CounterAggregator`
then folded back down to about twenty `Statistic` rows. That traffic — the inserts, the aggregator's
reads, and the deletes — was the largest single share of database execution time in a 100k-job run.

Increments now accumulate in a process-wide buffer and a flusher writes them out on an interval, one
row per distinct key. The same 100,000 jobs wrote **1,799 rows instead of two million**. Downstream
...
zero spins it and a negative value throws on every iteration into a catch that logs and retries.
...
1. **Policy attributes are now legal on job and message handler classes, on contracts, or on both — the handler wins.** The handler is often the right owner: "this handler talks to a single-connection legacy endpoint" is a statement about the handler, not about every publisher of the contract. Precedence is `explicit metadata at publish → handler → contract → global options`, resolved in one place, and resolution is **rung-major**: a handler `[Semaphore]` overrides a contract `[Mutex]` outright, because the two fill the same metadata slot. Placement on stream handlers and in-memory request handlers still fails, now at **build** time (`WARP001`).
2. **The resolved policy is written into `Job.Metadata` at the job's first execution, and from then on the row is the authority.** Later attempts read it and never re-resolve, so a retried or requeued job cannot change policy mid-flight and a redeploy never reshapes jobs already running — if a job did not behave as expected, open it and read what it says it will follow. Two consequences: **a job shows its policy only once its first attempt finishes** — nothing is stamped at publish except metadata you passed explicitly (at publish there is no handler to ask), and the stamp is persisted by the attempt's finalizing write — success, failure **and** graceful cancellation alike — so an `Enqueued` row is blank and a `Processing` row has resolved but not yet written. Stamping is **per family, not per job**: a behaviour that short-circuits (a `Skip`/`Wait` mutex, a throttled rate limit) returns before the behaviours nested inside it ever resolve, so a requeued job carries the family that decided its fate and resolves the rest on its next attempt. The dashboard's policy chips on a job's detail page read the same metadata, so they are empty until the first attempt has finished; the `warp.job.max_attempts` span tag is read after the pipeline resolved the budget, so a `[Retry]` declared on the handler shows on the first attempt's span as well; and **global defaults are never stamped** (an absent value means "the default applies"). Contract attributes used to be stamped at enqueue; if you have tooling that reads `Job.Metadata` on `Enqueued` rows to predict policy, it must move to reading the row after its first attempt.
3. **There is no longer any startup validation of policy placement, and declaring a policy on both axes is no longer an error.** `AddWarp` previously threw for the both-axes case, for `Scope = Total` on a handler, and for a handler `[Timeout]` under a `Total`-scoped global default. Both-axes is now a supported override. The remaining unresolvable placements fail the **build** where the compiler can see them (`WARP002`), and where it cannot they are **inert**: `[Timeout(Scope = Total)]` on a handler is ignored, the handler runs untimed, and Warp logs a warning once per request type — because a wall-clock budget measured from enqueue cannot be resolved once the job is already running (item 6 below states this correctly).
4. **Message handlers now honour all policy addons, resolved per child exactly like jobs.** Three activations at upgrade: (a) contract-declared policy on message types — previously stamped, copied to every child, and then ignored — becomes active, with **all children that declare nothing of their own contending on the one declared key**; (b) handler-declared policy applies to that handler's children only, overriding the contract for them; (c) **global defaults reach message handlers for the first time** — with `RetryOptions.MaxRetries > 0` configured, failing message handlers start retrying; with a `PerAttempt` default timeout, message handlers get timed out. Saga handlers are exempt (the saga proxy manages its own serialization and reschedules). Relatedly, a **`Deleted` child no longer blocks parent finalization**: a message/batch whose children are all settled (`Completed`/`Failed`/`Deleted`) finalizes. What a `Deleted` child means depends on the parent's kind. For a **message**, a child deleted by a Skip-mode `[Mutex]`/`[RateLimit]` is deliberately-skipped delivery, not failed work: the message completes when any handler completed and finalizes `Deleted` only when every child was skipped. For a **batch**, a deleted child is cancelled work: under the default `OnlyOnSucceeded` a batch with **any** deleted child (and no failed one) finalizes `Deleted` — so a **cancelled batch** (children bulk-deleted, batch row left `Processing`) now finalizes `Deleted` where it previously hung forever, and a **partially** cancelled batch (some children completed before the cancel) is never reported as a success either; its `OnlyOnSucceeded` continuation fails as the child of a deleted parent. A `Failed` child still finalizes the parent `Failed`. Under `OnAnyFinishedState` the parent finalizes `Completed` whatever its children did — the same rule that already applied to failed children — so the continuation activates. The breaker's **default group for a routed message child is its handler type**, not the message type: a message fans out to several handlers and one flaky handler must not open the circuit for its siblings (declare `[CircuitBreaker(Group = ...)]` to share one deliberately).
...
7. **`ConcurrencyPublishBehavior`, `RateLimitPublishBehavior` and `RetryPublishBehavior` were removed** (public types). They existed only to stamp contract attributes at publish, which is what made the handler axis unreachable. Explicit `WithMutex` / `WithSemaphore` / `WithRetry` / `WithRateLimit` / `WithTimeout` at publish are unaffected and still outrank every attribute — and they claim the **whole policy**, not a field of it: `WithRetry(5)` at publish means the retry policy is fully defined at that rung, so a `[Retry(3, Delays = [...])]` attribute contributes nothing beneath it and the global schedule applies. This is symmetric — setting only `RetryDelays` at publish claims the policy the same way, taking the global count beneath the explicit schedule, so an attribute can never overwrite a schedule the caller set explicitly. Policy is applied to **job-backed executions only**: an in-memory `IMediator.Send` of a type that happens to implement `IJob`/`IMessage` runs its handler directly, as before — there is no row to skip or reschedule.
...
- **Disabled recurring jobs no longer read as if they were still firing.** A disabled definition was never executed — the scheduler records a skipped firing and creates no job — but the skip path advanced `LastExecution` and `NextExecution` exactly like a real firing, so the dashboard showed both columns creeping forward on a job that had not run since it was disabled. (The contradiction was visible in the same row: **Last Result** filters skipped firings out, so it correctly showed nothing beside a ticking **Last Execution**.) `LastExecution` now moves only when a job actually ran, so **a definition left disabled will see that column freeze where it previously advanced**, and one that never ran keeps reading `Never`. `NextExecution` still advances internally — freezing it would leave the row permanently due and record one skipped entry per scheduler tick instead of one per cron occurrence — but the dashboard renders `—` for a disabled definition rather than a firing time that will not produce a job. The scheduler's server-task history entry also counts the two outcomes separately (`Scheduled 3 recurring jobs, skipped 1 disabled`) instead of reporting skips as scheduled work. Manual **Trigger** still overrides the disabled state, unchanged.
