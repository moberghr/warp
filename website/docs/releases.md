...
**The addon policy axis: `[Mutex]` / `[Semaphore]` / `[RateLimit]` / `[Timeout]` on handler classes, and full policy support for message handlers and recurring jobs.** No schema change. Several changes below alter behaviour a running deployment may be relying on — read 2, 3 and 5 before upgrading.

1. **Policy attributes are now legal on job and message handler classes** (`[Timeout]` handler-side is `PerAttempt` only). The handler is often the right owner — "this handler talks to a single-connection legacy endpoint" is a statement about the handler, not the contract. Handler-declared policy is resolved at first execution, stamped into `Job.Metadata` (the row explains itself; a redeploy never reshapes in-flight jobs), and enforced identically to contract-declared policy. Placement on stream handlers and in-memory request handlers still fails at startup (#242). `AddWarp` now throws when the same policy family is declared on **both** the contract and its handler (`[Mutex]`+`[Semaphore]` count as one family), when `Scope = Total` appears on a handler, and when a handler `[Timeout]` coexists with a `Total`-scoped global default.
...
3. **Message handlers now honour all policy addons.** Three activations at upgrade: (a) contract-declared policy on message types — previously stamped, copied to every child, and then ignored — becomes active, with **all handlers of the message contending on the one declared key**; (b) handler-declared policy applies to that handler's children only; (c) **global defaults reach message handlers for the first time** — with `RetryOptions.MaxRetries > 0` configured, failing message handlers start retrying; with a `PerAttempt` default timeout, message handlers get timed out. Saga handlers are exempt (the saga proxy manages its own serialization and reschedules). Relatedly, a **`Deleted` child no longer blocks parent finalization**: a message/batch whose children are all settled (`Completed`/`Failed`/`Deleted`) finalizes. Deleted children don't count as failures, but a parent whose settled children are **all** `Deleted` finalizes as `Deleted`, not `Completed` — so a **cancelled batch** (children bulk-deleted, batch row left `Processing`) now finalizes `Deleted` where it previously hung forever, and is never reported as a success.
4. **A `PerAttempt` global default timeout is applied at execution instead of stamped at publish.** Same effective timeout; the value no longer appears in `Job.Metadata` at enqueue, and it can no longer shadow a handler-declared `[Timeout]` (the #236 shape, fixed for Timeout). A `Total`-scoped default keeps publish stamping.
5. **Declaring `[Retry]` or `[CircuitBreaker]` on both a contract and its handler is now a startup error.** This configuration runs today with the handler side silently dead (shadowed by the publish-stamped contract value). The fix is deleting whichever attribute was not winning — behaviour is unchanged after the edit.
6. **`RetryAttribute`'s documented precedence is now true** — by making the ambiguous case unrepresentable rather than by reordering resolution.
