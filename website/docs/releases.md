...
Additive minor release — no breaking API changes, no migration. Introduces a **pluggable metric source** so Warp's dashboard and SLO evaluator can read their metrics back from **Prometheus** instead of the local `Statistic`/`Counter` fold — every page renders from the metrics Warp already exported via OpenTelemetry, keeping the operational database lean.

- **One read seam, two backends.** All aggregate metric reads (job execution stats, queue-wait, backlog, deadline, adapters, endpoints, client vitals, error-group trends) now flow through `IMetricSource`. The default `LocalMetricSource` reproduces the existing merged-read + tier down-bin semantics **exactly** — no numbers move. A new `Moberg.Warp.Metrics.Prometheus` package adds `PrometheusMetricSource`, which generates the PromQL itself from a logical metric catalog (there is no query editor). Select it with one call after `AddWarpServer`:

  ```csharp
  builder.Services.AddPrometheusMetricSource(o => o.BaseAddress = "http://prometheus:9090");
  ```
