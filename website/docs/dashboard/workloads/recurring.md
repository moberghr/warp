...
The list is built to be **scanned**: the **Schedule** column reads the cron expression back in plain English ("Every 15 minutes"), and **Next / Last Execution** answer "when, roughly?" ("in 10 minutes", "5 minutes ago"). Both
[tick live](/docs/dashboard/overview#timestamps-and-countdowns); **Next Execution** is a countdown,
so a definition whose scheduler has stopped reads `overdue by 5 minutes` rather than quietly
flipping to the past tense. In both cases the precise value — the raw expression, the exact timestamp — is on hover, and the timestamps render **to the minute**, since a cron occurrence is only ever minute-aligned.
