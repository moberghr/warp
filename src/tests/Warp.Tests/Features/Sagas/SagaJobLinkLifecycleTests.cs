...
public abstract class SagaJobLinkLifecycleTestsBase : IAsyncLifetime
{
    ...
    public async Task ForceComplete_DeletesSagaAndAllLinks_AtomicTransaction()
    {
        ...
        var command = new SagaCommandService<TestContext>(_fixture.CreateContext(), new FakeLockProvider(), Warp.Tests.Helpers.TestNotifiers.EmptyDispatcher(), NullLogger<SagaCommandService<TestContext>>.Instance);
    ...
    public async Task ForceComplete_MutexHeldByHandler_AbortsWithoutDeleting()
    {
        ...
        var command = new SagaCommandService<TestContext>(_fixture.CreateContext(), locks, Warp.Tests.Helpers.TestNotifiers.EmptyDispatcher(), NullLogger<SagaCommandService<TestContext>>.Instance);
        var removed = await command.ForceComplete(sagaId);
