...
public static class WarpDashboardServiceCollectionExtensions
{
    ...
    /// </summary>
    /// <remarks>
    /// Not needed for open access (<c>app.MapWarpUI("/warp")</c>) or when the host gates the dashboard
    /// on its own policy (<c>app.MapWarpUI("/warp").RequireAuthorization("YourPolicy")</c>).
    /// </remarks>
