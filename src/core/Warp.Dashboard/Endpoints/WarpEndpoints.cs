...
{
    /// <summary>
    /// Maps the dashboard's REST API. Returns the group so <c>MapWarpDashboard</c> can hand the host's endpoint
    /// conventions — <c>RequireAuthorization</c> and friends — to every endpoint in it.
    /// </summary>
    public static RouteGroupBuilder MapWarpApiEndpoints(this WebApplication app, WarpDashboardOptions options, List<IWarpDashboardExtension> extensions)
    {
