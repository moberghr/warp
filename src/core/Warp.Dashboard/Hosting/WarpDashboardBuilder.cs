...
public static class WarpDashboardBuilder
{
    /// <summary>
    ...
    /// </summary>
    public static WarpDashboardEndpointConventionBuilder MapWarpDashboard(this WebApplication app, string routePrefix)
    {
        ArgumentNullException.ThrowIfNull(app);

        return app.MapWarpDashboard(x => x.RoutePrefix = routePrefix);
    }

    /// <summary>
    /// Maps the Warp dashboard, taking options from DI and applying <paramref name="setupAction"/> on top.
    /// </summary>
    public static WarpDashboardEndpointConventionBuilder MapWarpDashboard(this WebApplication app, Action<WarpDashboardOptions>? setupAction = null)
    {
        ArgumentNullException.ThrowIfNull(app);

        WarpDashboardOptions options;
        using (var scope = app.Services.CreateScope())
        {
            options = scope.ServiceProvider.GetRequiredService<IOptionsSnapshot<WarpDashboardOptions>>().Value;
            setupAction?.Invoke(options);
        }

        return app.MapWarpDashboard(options);
    }

    /// <summary>
    /// Maps the Warp dashboard with explicit options.
    /// </summary>
    public static WarpDashboardEndpointConventionBuilder MapWarpDashboard(this WebApplication app, WarpDashboardOptions options)
    {
        ArgumentNullException.ThrowIfNull(app);
        ...
        var mapped = new WarpDashboardEndpointConventionBuilder(app.Services, shell, new CompositeEndpointConventionBuilder(api));
    ...
    private static void ApplyBuiltInLoginGate(WarpDashboardEndpointConventionBuilder mapped, WarpDashboardGate gate)
    {
    ...
    private static string NormalizeRoutePrefix(string routePrefix)
    {
        ...
        if (string.IsNullOrEmpty(trimmed))
        {
            throw new ArgumentException(
                "WarpDashboardOptions.RoutePrefix must name a path segment, e.g. \"/warp\". Mounting the dashboard at "
    ...
    private static void ClaimDashboard(IServiceProvider services, string routePrefix)
    {
        ...
        if (cookiePath.Value != null && services.GetService<IWarpDashboardLoginMarker>() != null)
        {
            throw new InvalidOperationException(
                $"MapWarpDashboard was called more than once (already mapped at \"{cookiePath.Value}\", now \"{routePrefix}\") "
